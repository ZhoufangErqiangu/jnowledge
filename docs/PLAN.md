...
| 维度 | 决策 |
|---|---|
| 真相源边界 | **按确定性切**：LLM 产出 = 原始事实，必落库；纯代码组装 = 派生逻辑，不入库、用时重算（DESIGN §8.2）。 |
...
| system prompt 可观测性 | 不整体入库；其依赖的 LLM 产出落 context_item，assembler 为纯函数重算（解 `chat.service.getContextDebug` 的 TODO）。 |
...
- [ ] 取代 `GENERATION_SYSTEM` / `agentDef.system` 静态常量，按上下文确定性组装。
- [ ] assembler 为**纯函数**，仅消费静态模板 + 已落库事实；debug 页据此重建任意历史轮的实际 system prompt（落实 §8.2 推论）。
- [ ] 稳定前缀前置、动态片段后置，保 DeepSeek 上下文缓存命中。
...
- **动态 system prompt**：debug 页能忠实重建任意历史轮的实际 system prompt。
