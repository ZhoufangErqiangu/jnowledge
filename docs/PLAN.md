...
- [ ] 取代 `GENERATION_SYSTEM` / `agentDef.system` 静态常量，按上下文确定性组装（assembler 在请求时**生产** system）。
- [ ] **实际发送值随轮快照落 context_item（`state=internal`, `meta.stage=system`）**；debug 页读快照重现任意历史轮的 system——**不事后重算**（assembler/模板是会迭代的代码，重算随版本漂移；落实 §8.2 修订）。
