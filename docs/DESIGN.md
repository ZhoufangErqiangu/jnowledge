...
与 §8.2 快照原则一致：后缀仍是 LLM 实际输入 → 仍随轮快照落库（`state=internal`，但 `meta.stage='scope'`，与前缀的 `stage='system'` 区分）。调试页 systemView 两条都收并带 `stage` 标签（前端从 raw 过滤 `meta.stage∈{system,scope}` 派生，见 §8.10）。可观测性不丢，只是拆成两条。
...
verdict 落 context_item 见 §8.3（第三状态首个试金石）。
...
2. **完整 raw（含 internal）全量下发，不设 server 隐私地板。** 依据：`active` / `internal` 是"视图整洁 / LLM 上下文管理"标记，**非保密标记**；且会话详情（`getConversation`）本就把全量 raw（含 internal、含 meta）下发前端，故 live 携带 internal **不新增泄露面**。会话级鉴权（SSE 已是 per-conversation authed）即边界。渲染与否由前端投影决定。
...
§8.9 让**聊天页**的用户视图完全由前端从 raw 派生（reload 与 live 同投影）。但**调试页**（`ContextDebugView`）当时仍走一条平行的服务端聚合端点 `GET /conversations/:id/context/debug`（`getContextDebug`），由后端预算调四个投影把 `{raw, runs, systemView, llmView, userView}` 一次算好下发。这与 §8.2 / §8.9 的分界**自相矛盾**：调试页展示的恰是"同一份真相源派生的各视图"，却是后端重算、前端纯展示——又一处"投影留在服务端"的残留。

> **原则收口**：调试页的每个视图都是 raw 上的**纯投影**，与聊天页同源同算。故调试页改为只取会话详情（`getConversation` 已下发 `{conversation, raw, runs}`，§8.9 阶段 4），四视图就地派生，后端 debug 端点连同其 DTO 一并删除。

四视图的派生（全部前端，输入是 `raw` 或 `raw.map(viewFromDebug)`）：

- **原始上下文** = `raw` 直取；**非主线推理** = `raw.filter(flags.state==='internal')`；
- **system 快照**（systemView）= `raw.filter(meta.stage∈{system,scope})` 本地映射 `{runId, label, stage, content}`——读的是**落库快照**（§8.2 忠实于发送当时），非事后重算；
- **用户视图** = `projectForUser(views)`（与聊天页**同一函数**）；
- **推理视图**（llmView）= `projectForChat(views)`，**文本近似**（仅 active 的 user/assistant 文本，不含 system、不含工具回合）。
